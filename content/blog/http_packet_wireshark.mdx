...
In our example above, the source port is 59423. My browser process used the Windows Sockets API to "bind" to this port. The destination port is port 443. Upon processing this TCP header, my server's operating system will pass the encapsulated data to  whichever software is "listening" to that port, which in this case is an nginx web server.
