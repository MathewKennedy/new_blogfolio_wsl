...
At the end of each row, we can see the corresponding ASCII characters for the bytes in that row, if they're printable. This is to allow us to quickly see any readable text in a packet. SYN packets don't generally contain readable text, but take a look at the ASCII from a later packet:
...
PAT also changes port numbers in the TCP header (which we'll look at next) in order to keep unique mappings in the router's Address Translation Table.
...
As mentioned above in the section on <a href="#source_destination_ip">Source and Destination IP Addresses</a>, as part of Port Address Translation, the source ports in this header will be replaced by your router. When the router receives responses, it will use its Address Translation Table to return the original source port to the header, ensuring your operating system can send data to port 59423 for the browser.
...
The SACK permitted option stands for "Selective Acknowledgement". Without this option, TCP uses "Cumulative Acknowledgement" as described in the <a href="#tcp_sequence_acknowledgement">Sequence Number and Acknowledgement Number</a> section. Cumulative acknowledgement essentially says "I've received everything up to this point". This means that if a packet is lost and resent out of sequence for any reason, the receiver will send an acknowledgement for that packet, and the sender will retransmit all data that followed the lost packet, even if it's already been received.
