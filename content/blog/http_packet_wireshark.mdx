...
🦈 [Wireshark](https://www.wireshark.org/about.html) is a network protocol analyser for macOS, Linux, and Windows. You can learn how to use Wireshark at [Wireshark Learn](https://www.wireshark.org/learn). This post aims to explain in its entirety a single Ethernet II frame capture containing a SYN packet.
...
Let's understand this dump first, starting with the four digit numbers on the left. These numbers are called <strong>offsets</strong>. They're added by Wireshark for our reference and are not actually contained in Ethernet frames.

Offsets are hexadecimal numbers that tell us how many bytes the current line is offset from the beginning of the frame. For example, the third line of binary is offset 16 bytes into the frame, so the offset is 0010 (16 in hexadecimal).
...
We're going to use two of Wireshark's panes to help us understand the content of each layer. In the <span className="text-[navy] dark:text-[skyblue]">packet detail</span> pane, Wireshark adds labels and looks up or infers more information for us using protocol standards. The <span className="dark:text-[bisque] text-[brown]">packet diagram</span> pane gives a visual layout of bytes.

## Ethernet II Header (Layer 2)

These are the 14 bytes in the Ethernet II header of our original frame:

```yaml /8c e5 ef 81 d0 1a f0 57 a6 9a c7 b6 08 00/#b /.......W....../#b
0000   8c e5 ef 81 d0 1a f0 57 a6 9a c7 b6 08 0045 00   .......W......E.
```

The packet diagram for Ethernet shows us what's in each byte:
...
MAC addresses are assigned to a Network Interface Controller (NIC) to give a device a unique address within a network. They facilitate the "hop to hop" communication between the two devices a frame is currently being passed to and from. In this case the destination MAC address belongs to a NIC in my home router, and the source MAC address belongs to a NIC in my computer.
...
The LG (Local/Global) bit is the second least significant bit (the second bit from the right) of the most significant byte (the first byte on the left) of the MAC address. It indicates whether the MAC address is locally administered (1) or globally unique (0). In this case both MAC addresses are globally unique.

-- difference / example of local / global --

The IG (Individual/Group) bit is the least significant bit (the first bit on the right) of the most significant byte of the MAC address. It indicates whether the MAC address is an individual (unicast) address (0) or a group (multicast/broadcast) address (1). In this case both MAC addresses are unicast.

-- difference / example of unicast/mutlicast/broadcst --
...
```yaml /45 00/#p /00 34 5a 29 40 00 80 06 00 00 c0 a8 01 1e 23 b2/#p /c2 4e/#p /.......W....../#p
0000   8c e5 ef 81 d0 1a f0 57 a6 9a c7 b6 08 00 45 00   .......W......E.
0010   00 34 5a 29 40 00 80 06 00 00 c0 a8 01 1e 23 b2   .4Z)@.........#.
0020   c2 4ee8 1f 01 bb ed 8c 65 fb 00 00 00 00 80 02   .N......e.......
```
Again, let's consult Wireshark's packet diagram for IPv4 to see what it contains.
...
We're using IP version 4 and the length of this IP header is 20 bytes. The 20 is actually sent as 5, as in the parenthesis above, meaning 5 * 4-byte words. This header length can vary, and the receiving device uses it to determine where to read from for the actual payload.
...
When a packet is created and sent, the TTL value is set to a default number, commonly 64, 128, or 255, depending on the operating system or network configuration. Each time the packet is processed by a hop, the TTL value is decremented by 1. When it reaches 0, the packet is discarded. TTL is also useful in network diagnostics, where tools like "traceroute" use TTL to to determine the intermediate routers along a path.

<h3 className="text-[navy] dark:text-[skyblue]">Protocol</h3>
This indicates which protocol is encapsulated within this IP header, much like the EtherType field did for the layer 2 Ethernet header. In our case, that's TCP (Transmission Control Protocol), represented by the number 6.
...
As mentioned above, as part of Port Address Translation, the ports in this header will be replaced by your router. When the router receives responses, it will use its Address Translation Table to return the original ports to the header, ensuring your operating system can send data to the correct service.
...
"Conversation completeness" is calculated using the TCP flags, so we can discuss it in detail in the <a href="#flags">"Flags"</a> section.

"TCP segment length" is 0, because there's no actual data encapsulated in this SYN TCP header. The "three way handshake" we're in uses data in the TCP header itself.
...
<h3 className="text-[navy] dark:text-[skyblue]">TCP Flags</h3>
