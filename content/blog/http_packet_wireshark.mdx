...
This indicates which protocol is encapsulated within this IP header, much like the EtherType field did for the layer 2 Ethernet header. In our case, that's TCP (Transmission Control Protocol).
...
Finally, we have the source and destination IP addresses. Where MAC addresses facilitied hop-by-hop communication, IP addresses facilitate "end-to-end" communication.

Here, the desination address is the *public IP address* of one of my servers. The source address is a *private IP address* assigned to my computer by my home router. Private IP addresses are not used on the public internet, as they're not unique, so when my router processes this frame, it will replace the source IP in this header with its own unique public IP address using Port Address Translation (PAT). PAT also changes port numbers in the TCP header we're going to look at next, in order to keep unique mappings in the router's Address Translation Table. Using ports ensures the router can differentiate connections to the same IP address.
...
```yaml /e8 1f 01 bb ed 8c 65 fb 00 00 00 00 80 02/ /......e......./ /fa f0 a7 ed 00 00 02 04 05 b4 01 03 03 08 01 01/ / ................/ /04 02/ / ../
