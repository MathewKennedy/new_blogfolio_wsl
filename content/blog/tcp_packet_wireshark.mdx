...
As mentioned above, my router changes the source port in outbound TCP headers as part of Port Address Translation. This ensures unique mappings between private and public IP addresses in the router's Address Translation Table. When my router receives responses, it will use the Address Translation Table to return the original port and IP address to the response TCP headers, ensuring my operating system can send data to <span className="text-[pink]">192.168.1.30</span>:<span className="text-[bisque]">59423</span> for the browser.
...
The "No-Operation" options are 1 byte options used to "pad" the data so that all options are 32-bits in length. 32 bit options can be parsed more efficiently by modern software and hardware. This is why the IP and TCP Header Length fields also multiply 4-byte (32 bit) words.